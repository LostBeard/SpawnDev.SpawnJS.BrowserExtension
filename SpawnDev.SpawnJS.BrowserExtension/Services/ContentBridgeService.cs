...
namespace SpawnDev.SpawnJS.BrowserExtension.Services
{
    ...
    public class ContentBridgeService : IAsyncBackgroundService
    {
        ...
        SpawnJSRuntime JS;
        public ContentBridgeService(SpawnJSRuntime js, BrowserExtensionService browserExtensionService)
        {
            JS = js;
        ...
        async Task InitAsync()
        {
            if (BrowserExtensionService.ExtensionMode == ExtensionMode.Content)
            {
                ...
                await ContentSiteContextScriptLoader("_content/SpawnDev.SpawnJS.BrowserExtension/content-bridge.js");
                await ContentSiteContextScriptLoader($"_content/SpawnDev.SpawnJS.BrowserExtension/content-bridge-loader.js?instanceId={remoteInstanceId}&remoteInstanceId={instanceId}");
            }
        }
