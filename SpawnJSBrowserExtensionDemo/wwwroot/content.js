// Todd Tanner 2026
// Loads .Net WASM app into the extension "content" context of an existing webpage
var initNetContent = async function () {
    var verboseMode = true;
    var consoleLog = function () {
        if (!verboseMode) return;
        console.log(...arguments);
    };
    // .Net will fail to load due to ContentSecurityPolicy (CSP) restrictions on some websites
    ...
    // the background worker can optionally patch the website's CSP rules to allow .Net to run or do nothing
    ...
    async function WebAssemblyCompileStreaming() {
        var ret = null;
        var args = [...arguments];
        consoleLog('WebAssemblyCompileStreaming called:', args);
        try {
            ...
            // then just async hang here to prevent .Net from throwing an error as it is up to the background worker now
            await new Promise(() => { });
        }
    }
    ...
    // This will cause .Net loading to fail
    ...
    function onSecurityPolicyViolation(e) {
        ...
        if ((e.blockedURI === "wasm-eval" || e.blockedURI === "wasm-unsafe-eval")
            ...
            consoleLog('.Net onSecurityPolicyViolation detected', cspViolation);
            chrome.runtime.sendMessage({ cspViolation });
        }
    }
    document.addEventListener('securitypolicyviolation', onSecurityPolicyViolation);

    var extensionAppBasePath = chrome.runtime.getURL('/app/');
    ...
    if (isMozilla) {
        ...
        // .Net will try to use globalThis.addEventListener, point it at window's
    ...
    globalThis.fetch = async function (resource, options) {
        ...
        if (xrayWrapped || isMozilla) {
            ...
            // which .Net will do when starting up.
    ...
    if (useModule) {
        script.type = 'module';
        script.src = chrome.runtime.getURL('app/main.module.js');
    } else {
        script.src = chrome.runtime.getURL('app/main.classic.js');
    }
    const promise = new Promise((res, rej) => {
        script.onload = () => res();
        script.onerror = () => res();
    });
    // Append to the page DOM so it executes in the page context
    (document.head || document.documentElement).appendChild(script);

    try {
        consoleLog('.Net content loading......');
        await promise;
        consoleLog('.Net content loaded');

    } catch (ex) {
        consoleLog('.Net content load failed...', ex);
    }
};
initNetContent();
